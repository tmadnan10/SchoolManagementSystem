          ...
          <section id="blog" class="section container blog-columns blog-preview">
            <div class="row">
              ...
              <header class="sec-heading">
                <h2>Class Tests</h2>
                <span class="subheading">Currently you have {{count($CTs)}} Uploaded Class Test(s)</span>
              </header>
@if (count($CTs) > 0)
   ...
   print date( 'd M Y', $timestamp );?></div></td>
                                        <td class="table-text"><div><a href="{{url('../')}}/{{ $task->marks }}">{{ $task->marks }}</a></div></td>
                                        <!-- Task Delete Button -->
